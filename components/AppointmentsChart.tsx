import { useMemo, useState } from "react";
import { LayoutChangeEvent, Platform, StyleSheet, Text, View } from "react-native";
import Svg, { Line, Rect, Text as SvgText } from "react-native-svg";
import { dateKey, daysInMonth, MONTH_INITIALS } from "../lib/dayGrid";
import { useThemePalette } from "../lib/ThemeContext";
import { theme } from "../lib/theme";

const CHART_FONT = Platform.OS === "web" ? "system-ui, -apple-system, Segoe UI, Roboto, sans-serif" : undefined;
const H = 120;
const PAD_X = 8;
const PAD_TOP = 12;
const PAD_BOTTOM = 20;

export default function AppointmentsChart({
  year,
  appointmentsByDay,
}: {
  year: number;
  appointmentsByDay: Map<string, number>;
}) {
  const palette = useThemePalette();
  const COLOR = palette.accentBlue;
  const [width, setWidth] = useState(0);

  const days = useMemo(() => {
    const list: { index: number; key: string; count: number }[] = [];
    let index = 0;
    for (let m = 0; m < 12; m++) {
      for (let d = 1; d <= daysInMonth(year, m); d++) {
        const key = dateKey(year, m, d);
        list.push({ index, key, count: appointmentsByDay.get(key) ?? 0 });
        index += 1;
      }
    }
    return list;
  }, [year, appointmentsByDay]);

  const monthStarts = useMemo(() => {
    const starts: { label: string; index: number }[] = [];
    let index = 0;
    for (let m = 0; m < 12; m++) {
      starts.push({ label: MONTH_INITIALS[m], index: index + daysInMonth(year, m) / 2 });
      index += daysInMonth(year, m);
    }
    return starts;
  }, [year]);

  const total = useMemo(() => [...appointmentsByDay.values()].reduce((a, b) => a + b, 0), [appointmentsByDay]);

  const n = days.length;
  const plotW = Math.max(1, width - 2 * PAD_X);
  const plotBottom = H - PAD_BOTTOM;
  const xFor = (i: number) => PAD_X + (n > 1 ? (i / (n - 1)) * plotW : 0);
  const maxCount = Math.max(1, ...days.map((d) => d.count));
  const yFor = (c: number) => plotBottom - (c / maxCount) * (plotBottom - PAD_TOP);
  const barW = Math.max(2.5, plotW / n);

  return (
    <View style={styles.card}>
      <Text style={styles.title}>Buyer / Seller Appointments</Text>
      <Text style={styles.subtitle}>Check-ins tagged Buyer or Seller, per day, {year}.</Text>

      <View onLayout={(e: LayoutChangeEvent) => setWidth(e.nativeEvent.layout.width)} style={{ height: H }}>
        {width > 0 ? (
          <Svg width={width} height={H}>
            <Line x1={PAD_X} y1={plotBottom} x2={width - PAD_X} y2={plotBottom} stroke={palette.border} strokeWidth={1} />
            {days
              .filter((d) => d.count > 0)
              .map((d) => {
                const y = yFor(d.count);
                return (
                  <Rect key={d.key} x={xFor(d.index) - barW / 2} y={y} width={barW} height={plotBottom - y} fill={COLOR} rx={Math.min(2, barW / 2)} />
                );
              })}
            <SvgText x={PAD_X} y={PAD_TOP + 2} fill={palette.textMuted} fontSize={9} fontFamily={CHART_FONT}>
              {`${maxCount}`}
            </SvgText>
            {monthStarts.map((mo, i) => (
              <SvgText key={i} x={xFor(mo.index)} y={H - 4} fill={palette.textMuted} fontSize={9} fontFamily={CHART_FONT} textAnchor="middle">
                {mo.label}
              </SvgText>
            ))}
          </Svg>
        ) : null}
      </View>

      {total === 0 ? <Text style={styles.note}>No buyer or seller check-ins in {year} yet.</Text> : null}
    </View>
  );
}

const styles = StyleSheet.create({
  card: {
    backgroundColor: theme.color.surface,
    borderWidth: 1,
    borderColor: theme.color.border,
    borderRadius: theme.radius.md,
    padding: theme.spacing(4),
    marginTop: theme.spacing(4),
  },
  title: { color: theme.color.textPrimary, fontSize: theme.font.subtitle, fontWeight: "700" },
  subtitle: { color: theme.color.textSecondary, fontSize: theme.font.caption, marginTop: 2, marginBottom: theme.spacing(2) },
  note: { color: theme.color.textMuted, fontSize: theme.font.caption, marginTop: theme.spacing(2), textAlign: "center" },
});
